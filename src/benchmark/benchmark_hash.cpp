...
void BenchmarkHash::benchmark_murmurhash2A() {
    ...
    std::cout << "\t" << "MurmurHash2A: " << m_num_ops / best_time / (1000 * 1000) << std::endl;
}

void BenchmarkHash::benchmark_murmurhash3() {
    double start_time, end_time, best_time;
    uint32_t hashcode = 0;
    best_time = 1e30;
    for (int i = 0; i < 10; i++) {
        start_time = CycleTimer::currentSeconds();
        for (int j = 0; j < m_num_ops; j++) {
            MurmurHash3_x86_32(m_keys[i].c_str(), m_keys[i].length(), 0, &hashcode);
        }
        end_time = CycleTimer::currentSeconds();
        best_time = std::min(best_time, end_time-start_time);
    }
    std::cout << "\t" << "MurmurHash3: " << m_num_ops / best_time / (1000 * 1000) << std::endl;
}
