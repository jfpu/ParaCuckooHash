...
const static std::string empty_key = "-invalid";

template <typename T>
BenchmarkDenseHashMap<T>::BenchmarkDenseHashMap(
...
template <typename T>
void BenchmarkDenseHashMap<T>::benchmark_random_interleaved_read_write() {
  ...
  /*
  	for (float space_efficiency = 0.15f; space_efficiency <= 0.9f; space_efficiency += 0.15f) {
	    ...
	    for (float read_percentage = 0.0f ; read_percentage <= 1.0f; read_percentage += 0.10f){
          ...
          google::dense_hash_map<T, T> my_map;
          my_map.set_empty_key(empty_key);
          my_map.resize(num_buckets);
...
template <typename T>
void BenchmarkDenseHashMap<T>::benchmark_read_only() {

    // CuckooFineHashMap<T> my_map(m_num_buckets);
    google::dense_hash_map<T, T> my_map;
    my_map.set_empty_key(empty_key);
    my_map.resize(m_num_buckets);
...
template <typename T>
void BenchmarkDenseHashMap<T>::benchmark_write_only() {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        for (int j = 0; j < m_num_ops; j++) {
            my_map.emplace(m_random_keys[j], m_random_keys[j]);
            // my_map[m_random_keys[j]] = m_random_keys[j];
        }
...
template <typename T>
void BenchmarkDenseHashMap<T>::benchmark_read_only_single_bucket() {
	...
	for (int i = 0; i < m_num_ops; i++) {
		my_map.emplace(m_random_keys[i], m_random_keys[i]);
		// my_map[m_random_keys[i]] = m_random_keys[i];
	}
	    ...
	    for (int i = 0; i < 10; i++) {
		    ...
		    for (int j = 0; j < num_readers; j++) {
		        pthread_create(&workers[j], NULL,
		        	thread_send_requests_for_emplace_find<google::dense_hash_map<T, T>>, &args[j]);
		    }
		    for (int j = 0; j < num_readers; j++) {
...
template <typename T>
void BenchmarkDenseHashMap<T>::benchmark_space_efficiency() {
	...
	for (float space_efficiency = 0.15f; space_efficiency <= 0.9f; space_efficiency += 0.15f) {
        ...
        std::cout << "\t" << 100*space_efficiency << "% Space Efficiency: (" << NUM_READERS << " Reader Threads): " <<(1000 * 1000) * best_time / m_num_ops / NUM_READERS << std::endl;
	}
}

template <typename T>
double BenchmarkDenseHashMap<T>::m_benchmark_reads_helper(
       google::dense_hash_map<T, T>* my_map, float read_percentage, float ops_percentage) {
...
template <typename T>
void BenchmarkDenseHashMap<T>::run_all() {

	std::cout << "Benchmarking DenseHashMap, " << m_num_ops << " Operations..." << std::endl;
