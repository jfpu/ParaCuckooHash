...
template <typename T>
BenchmarkLibCuckooHashMap<T>::BenchmarkLibCuckooHashMap(
...
template <typename T>
void BenchmarkLibCuckooHashMap<T>::benchmark_random_interleaved_read_write() {
  ...
  /*
  	for (float space_efficiency = 0.15f; space_efficiency <= 0.9f; space_efficiency += 0.15f) {
	    ...
	    for (float read_percentage = 0.0f ; read_percentage <= 1.0f; read_percentage += 0.10f){
          ...
          libcuckoo::cuckoohash_map<T, T> my_map;
...
template <typename T>
void BenchmarkLibCuckooHashMap<T>::benchmark_read_only() {

    // CuckooFineHashMap<T> my_map(m_num_buckets);
    libcuckoo::cuckoohash_map<T, T> my_map;
    my_map.reserve(m_num_buckets);
...
template <typename T>
void BenchmarkLibCuckooHashMap<T>::benchmark_write_only() {
...
template <typename T>
void BenchmarkLibCuckooHashMap<T>::benchmark_read_only_single_bucket() {
	    ...
	    for (int i = 0; i < 10; i++) {
		    ...
		    for (int j = 0; j < num_readers; j++) {
		        pthread_create(&workers[j], NULL,
		        	thread_send_requests_for_insert_find<libcuckoo::cuckoohash_map<T, T>>, &args[j]);
		    }
		    for (int j = 0; j < num_readers; j++) {
...
template <typename T>
void BenchmarkLibCuckooHashMap<T>::benchmark_space_efficiency() {
...
template <typename T>
double BenchmarkLibCuckooHashMap<T>::m_benchmark_reads_helper(
       libcuckoo::cuckoohash_map<T, T>* my_map, float read_percentage, float ops_percentage) {
...
template <typename T>
void BenchmarkLibCuckooHashMap<T>::run_all() {

	std::cout << "Benchmarking Libcuckoo, " << m_num_ops << " Operations..." << std::endl;
